...
def build():
    ...
    if saltedtype=="density":
        ...
        builder = PsiBuilder(rank, system=(species, lmax, nmax, lmax_max, nnmax, ndata,
                                           atomic_symbols, natoms, natmax),
                             atom_info=(atom_idx, natom_dict))

        if rank == 0: print(f"problem dimensionality: {builder.totsize}", flush=True)
        ...
        for iconf in conf_range:
            ...
            ), builder.build(iconf, frames[iconf]))
