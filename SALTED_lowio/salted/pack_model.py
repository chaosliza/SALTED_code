...
def first_match(pattern):
    # sorted(): without it "first" means whatever the filesystem returned first,
    # so with two regularisations in one directory the packed model was
    # arbitrary AND could differ between runs over the same data.
...
def pack_averages(SALTED_file, path, debug: bool = False):
    ...
    # sorted(): glob returns filesystem order, so two numerically identical runs
    # packed AVERG in different orders and produced .salted files differing by
    # ~1.2 kB. Harmless to NoSpherA2, which reads this block by key, but it
    # defeats comparing models by hash. PROJ and FEATS already sort their keys.
...
def pack_wigners(SALTED_file, path, inp, debug: bool = False):
    ...
    # basename(): this split the FULL PATH on "_", so any underscore in
    # saltedpath shifted the index and raised IndexError. Lisa's real path
    # (train_models/GPR_parameters/...) crashes the packer outright; it only
    # ever worked because the job packs from /model in node-local scratch.
...
# VERSION 3 adds the optional NORMC block (electron-count constraint). It is
# backward compatible: NoSpherA2 validates no version number and walks the
# table of contents by name, so an older build reads a V3 file and simply
# ignores the block it does not know.
VERSION = 3
HAS_PYSCF = False
try:
    import pyscf
...
def pack_charge_constraint(SALTED_file, inp, debug: bool = False):
    """Electron-count constraint directive.

    Measured on this training set: the RI reference coefficients themselves
    integrate to -0.208 % of the exact electron count, and the ML prediction
    to -0.235 %. So ~88 % of the deficit is inherited from the density fit,
    not learned - which is why this is corrected at application rather than
    penalised in the loss.

    Deliberately NOT per-species. The atom-centred auxiliary basis is not an
    atomic-charge partition: measured per-element ratios run from 0.899 (Al)
    to 1.150 (B) with up to 10 % scatter, which is bonding, not error. Only
    the total is meaningful, so the constraint is global.
    """
    if debug:
        print('Writing Charge Constraint')
    begin_of_block = SALTED_file.tell()
    SALTED_file.write(i32(int(types_dict['float64'])))
    entries = {
        'MODE': float(inp.gpr.charge_correction_mode),
        'DEFCT': -0.00208,   # measured mean relative deficit of the reference fit
        'NCAL': 64.0,        # structures the above was measured over
    }
    SALTED_file.write(i32(len(entries)))
    for key, val in entries.items():
        write_key5(SALTED_file, key.encode('utf-8'))
        arr = np.asarray([val], dtype='<f8')
        write_data_head(SALTED_file, arr)
        SALTED_file.write(arr.tobytes())
    write_chunk_location(SALTED_file, 'NORMC', begin_of_block)


BLOCKS = ["AVERG", "WIG", "FPS", "FEATS", "PROJ", "WEIGH", "CONFG", "NORMC"]
...
def build(debug: bool = False):
    ...
    with open(f"{inp.salted.saltedname}.salted", "wb") as f:
        ...
        pack_charge_constraint(f, inp, debug)
        if HAS_PYSCF:
